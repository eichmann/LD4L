...
//	int maxCrawlerThreads = 1;
	...
	protected static String generatePayload(String queryIRI) throws MalformedURLException, IOException, InterruptedException {
		return generatePayload(queryIRI, prefix + prop_file.getProperty("query"));
	}

	protected static String generateSubauthorityPayload(String queryIRI, String subauthority) throws MalformedURLException, IOException, InterruptedException {
		return generatePayload(queryIRI, prefix + prop_file.getProperty(subauthority+ "Query"));
	}

	protected static String generatePayload(String queryIRI, String query) throws MalformedURLException, IOException, InterruptedException {
		StringBuffer buffer = new StringBuffer();

		ParameterizedSparqlString parameterizedString = new ParameterizedSparqlString(query);
	...
	protected static void annotateLoCName(String URI, Document theDocument, String predicate1, String predicate2) {
		String query = "SELECT ?value WHERE { " + "<" + URI + "> <http://www.loc.gov/mads/rdf/v1#" + predicate1
				+ "> ?o . " + "?o <http://www.loc.gov/mads/rdf/v1#" + predicate2 + "> ?value . " + "} ";
